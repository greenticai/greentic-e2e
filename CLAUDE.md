...
1. **Nightly Install/Wizard** (`nightly-e2e.yml`, 00:00 UTC) - Tests `gtc install`, `gtc doctor`, and `gtc wizard` across 6 platform/arch combos (Linux x64/arm64, macOS arm64/x64, Windows x64/arm64). Uses `expect` scripts for interactive wizard testing.
   ...
   **`Run gtc tenant install` soft-skips a fixed list of upstream release-artifact breakages** — things this repo cannot fix, where a red nightly reports someone else's pipeline. Each entry names one exact signature and is removed when that artifact is fixed; the step emits a `::warning::` naming which one matched, so a muted breakage stays visible in the run summary. The classifier is tested (`scripts/test_nightly_tenant_softskip.sh`), including that it does NOT skip an unrelated failure.

   **An entry is removed the moment upstream ships, and the fixture stays.** greentic-component's `*-pc-windows-msvc.zip` assets were POSIX tar archives through v1.1.4 — cargo-binstall could not extract them, fell back to a source build, and that build collided with an already-installed binary, so `gtc install --tenant` was broken for real Windows users, not just CI. Fixed upstream in greentic-component **1.1.5** (its packaging used `tar -a -cf x.zip`, which under GNU tar writes an uncompressed tar because `-a` selects a compression filter and knows no zip container). The soft-skip entry is gone, and its test now asserts the signature **fails** the job — the captured log is kept as a fixture precisely so a silent recurrence is caught rather than absorbed by a mute nobody remembers. That is the intended lifecycle for every entry in this list.
