...
- `scripts/run_type_only_routing_e2e.sh` - Track-A guard for greentic-runner #611. Uses `fixtures/packs/type-only-routing-probe` (two same-type flows: `entry_main` entry, `internal_helper` internal). Drives a type-only WebChat ingress and asserts a bot reply arrives, the runtime never logged the ambiguity bail, and `internal_helper` never won the route. Env: `PORT`, `WORK_DIR_BASE`, `SKIP_BUILD`, `KEEP_BUNDLE`.
