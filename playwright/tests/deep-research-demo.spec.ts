...
test.describe("deep-research-demo (PR-1 walking skeleton)", () => {
  ...
  // Bot reply requires LLM API key (Chat2Flow routing). Skipped when absent.
  test("functional: bot replies (skipped when LLM key absent)", async ({
    page,
    gtcDemo,
  }) => {
    const demo = await gtcDemo({
      name: "deep-research-demo",
    ...
    await chat.send("Tell me about renewable energy");

    const reply = await chat.awaitReply({ minLength: 50, timeoutMs: 90_000 });
